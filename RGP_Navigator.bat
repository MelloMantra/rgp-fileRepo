...
echo.
echo "e" to exit, "h" for help, or "a" for advanced options.
echo Consoles:
echo 1: NES
echo 2: GBA
echo 3: GBC
...
exit
:help
cd "%packPath%\Contents\assets\" && echo User accesses troubleshooting manual on %date% at %time%.
start /wait troubleshooting.txt 
cls && goto consoleQuery
:updateSoftware
echo               UPDATING...
start "%cd%\RGP Setup.bat"
exit